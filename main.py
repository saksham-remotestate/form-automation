...
def build_task(url: str, photo_path: str, pdf_path: str):
    ...
    return f"""
You are an advanced automated browser agent with full DOM, vision, and reasoning capabilities.
...
- Prefer visible, interactive fields.
- Infer labels and expected input types using multi-signal analysis.
- Never try to proceed without filling compulsory fields.
...
- Insurance? → {"Yes" if step1['insurance'] else "No"}
...
{chr(10).join([f"- Person {i+1}: Relationship → {person['relationship']}, First Name → {person['first_name']}, Surname → {person['surname']}, Passport → {person['travel_document_number']}" for i, person in enumerate(step4['representative']['companion']['people'])])}
...
----------------------------------------
Fields to identify and fill USING EXACT VALUES:
- "Citizenship / Country" → {step1['citizenship']}
- "Type of Travel Document / Passport" → {step1['travel_document_type']}
- "Purpose of visit" → {step1['purpose_of_visit']}
- "Insurance?" → {"Yes" if step1['insurance'] else "No"}
- Confirmations:
    - IDENTIFY the checkbox whose input name matches "HasReadBorderCrossInfo" and label text contaains "I confirm that I have read and understood terms and conditions for entering territory Georgia.:
    - Use this priority to find the checkbox: <label for="..."> matching text, aria-labelledby, aria-label, title, nearby visible text.
    - - Scroll into view, ensure interactable, then click or set checked=true.
- CAPTCHA:
    - Solve it using OCR rules defined earlier.
- Only press **Next** when all required elements are valid and the confirmation checkbox remains verified checked.
- Never attempt to press Next, Continue, or any navigation control until the confirmation checkbox verification step above completes successfully.
...
# REPRESENTATIVE MODE:
# - If representative is enabled in data, look for "Application submitted by representative" checkbox/section
...
# COMPANION MODE:
# - If companion is enabled with {len(step4['representative']['companion']['people'])} people, look for "Travelling With Companion" section
# - Fill companion fields for each person:
# {chr(10).join([f'    • Person {i+1}: Relationship → {person["relationship"]}, First Name → {person["first_name"]}, Surname → {person["surname"]}, Passport → {person["travel_document_number"]}' for i, person in enumerate(step4['representative']['companion']['people'])])}
# - Click "Add person" if required for multiple companions.
...
async def main():
    ...
    try:
        ...
        print(f"Could not get photo URL: {e}")
    ...
    agent = Agent(task=task, llm=llm, available_file_paths=[
                  photo_path, pdf_path], generate_gif=True)
